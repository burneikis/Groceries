...
export default function ItemRow({ item }) {
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-3 border-t border-gray-50 group">
      <button
        onClick={handleCheck}
      ...
      >
        <span className={`text-sm transition-colors duration-200 ${isChecked ? 'line-through text-gray-400' : 'text-gray-900'}`}>
          {item.name}
        </span>
        {item.amount && (
          <span className="ml-2 text-xs text-gray-400">{item.amount}</span>
        )}
        {item.description && (
      ...
      <button
        ...
        className="opacity-100 sm:opacity-0 sm:group-hover:opacity-100 p-1 text-gray-300 hover:text-red-400 active:text-red-400 transition-all"
      >
